...
const ViewRoom = () => {
  ...
  const handleDeleteRoomFeature = async (roomId, featureId) => {
    const confirmation = window.confirm("Are you sure you want to delete this feature?");
    if (!confirmation) {
      return;
    }
  
    try {
      const response = await axiosInstance.delete(`/api/delete/room/${roomId}/feature/${featureId}/`);
  
      // Handle the response from the server
      if (response.status === 200) {
        // Filter out the deleted feature from the features list
        const updatedFeatures = reservationDeatils.features?.filter(
        ...
        setReservationDetails((prevDetails) => ({
          ...prevDetails,
          features: updatedFeatures, // Set the updated features
        }));
