...
const EditRoom = () => {
  ...
  const [rows, setRows] = useState([{ image: "", id: null }]);
  ...
  //       const filteredAvailableFeatures = availableFeatures.filter(
  //         (availableFeature) =>
  //           !chosenFeatures.features.some(
  //             (chosenFeature) => chosenFeature === availableFeature // Assumes both have the same structure or value
  //           )
  //       );

  //       // Set available features to only those that haven't been selected
  //       setAvailableFeatures(filteredAvailableFeatures);
  ...
  const fetchRoomByID = async () => {
    try {
      const response = await axiosInstance.get(`/api/edit-room/${id}/`);
      if (response.status === 200) {
        ...
        const formattedImages = data.images.map((imageUrl) => ({
          image: imageUrl.room_image,
          id: imageUrl.id,
        }));
        setRows(formattedImages);
        const availableResponse = await axiosInstance.get(
          "/api/get_feature/list/"
        );
        const availableFeatures = availableResponse.data;
        if (availableResponse.status === 200) {
          const filteredAvailableFeatures = availableFeatures.filter(
            (availableFeature) =>
              !data.features.some(
                (chosenFeature) =>
                  chosenFeature.feature_name === availableFeature.feature_name // Assumes both have the same structure or value
              )
          );
  ...
  const handleSubmit = async (e) => {
    ...
    // Append images
    rows.forEach((row) => {
      // Assuming `row` has properties `image` and `id`
      if (row.id === null && row.image instanceof File) {
        // Image to be added
        formData.append("images_to_add", row.image); // Append the image file for adding
      } else if (row.image instanceof File) {
        // Image to be updated
        formData.append("images_to_update", row.image); // Append the image file for updating
        formData.append("image_ids", row.id); // Append the corresponding ID
      }
    });
    try {
  ...
  return (
    <Sidebar>
      <div className="flex flex-col">
        <h1 className="text-2xl font-medium text-center  pb-2">Edit Room</h1>
