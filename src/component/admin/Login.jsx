...
import { setToken, setEmail, setUsername } from "../authSlice"; // Update the path to your authSlice
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!email || !password) {
      setError("Both filed are required.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch(setToken(data.token)); // Store token
        dispatch(setEmail(email)); // Store email (if needed)
        dispatch(setUsername(data.username));
        // Redirect user to dashboard or another page
        window.location.href = "/admin/dashboard"; // Redirect to a new route
        setLoading(false);
      } else {
