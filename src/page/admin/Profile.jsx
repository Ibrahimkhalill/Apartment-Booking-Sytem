...
import { logout } from "../../component/authSlice";
import LogoutModal from "../../component/admin/LogoutModal";
...
const Profile = () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    // Basic validation
    if (!newPassword || !oldPassword || !confirmPasword) {
      alert("Both filed are required.");
      return;
    }
    if (newPassword !== confirmPasword) {
      alert("Password did't Macth");
      return;
    }

    try {
      // Sending a POST request to the backend (adjust the URL to match your API)
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/pasword/changes`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ oldPassword, newPassword }), // Sending email and password as JSON
        }
      );
            ...
            <div className="flex items-center justify-center flex-col gap-2">
              ...
              <div className="flex items-center justify-center w-full mt-4">
                <button
                  ...
                  className="uppercase text-sm bg-[#2e9e28] text-white w-full flex items-center justify-center gap-2 py-2 rounded-md"
                >
